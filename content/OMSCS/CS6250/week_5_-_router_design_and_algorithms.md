...
- **Input queue**: responsible for finishing the [layer 1](../../notes/layer_1_physical.md) process and [de-encapsulation](../../notes/encapsulation.md) the message to get the [IP address](../../notes/internet_protocol_(ip).md). The it looks up where it has to go and moves it onto the switching fabric.
...
The control plane refers to the implementation of the [router](../../notes/router.md) [protocols](../../notes/protocol_(networks).md). This mainly involves keeping the forwarding table up to date.
...
- **Lookup**: When a packet arrives this uses the [IP address](../../notes/internet_protocol_(ip).md) to find the output link in the [Forwarding information base (FIB)](../../notes/forwarding_information_base_(fib).md). It uses longest prefix matching to do this. Some routers offer more complicated lookups using [ports](../../notes/port.md) and other criteria.
...
- **Header validation and checksum**: Check the packet, update [TTL](../../notes/time_to_live_(ttl).md) and recalculate the checksum.
- **Route processing**: Building the [Forwarding information base (FIB)](../../notes/forwarding_information_base_(fib).md). This uses [protocols](../../notes/protocol_(networks).md) such as [RIP](../../notes/routing_information_protocol_(rip).md), or [OSPF](../../notes/open_shortest_path_first_(ospf).md).
- **Protocol processing**: The routers need to implement: [SNMP](../../notes/simple_network_management_protocol_(snmp).md), [TCP](../../notes/transmission_control_protocol_(tcp).md), [UDP](../../notes/user_datagram_protocol_(udp).md), and [ICMP](../../notes/internet_control_message_protocol_(icmp).md) for sending error messages.
...
**Longest prefix matching** Matching a packet to the [FIB](../../notes/forwarding_information_base_(fib).md) can be complicated. New routers offer more complicated lookups as well.
...
Matching addresses to entries in the [FIB](../../notes/forwarding_information_base_(fib).md) is one of the most time sensitive and mathematically hard operations in a router.
...
- **Slash notation**: We provide the [IP address](../../notes/internet_protocol_(ip).md) followed by a slash and the size of the relevant prefix in [bits](../../notes/bit.md). i.e. 123.0.0.0/8.
- **Masking**: We provide the [IP address](../../notes/internet_protocol_(ip).md) with the [network mask](../../notes/network_mask.md).
...
We can use [subnet](../../notes/subnets.md) to form a [trie](../../notes/trie.md) over the alphabet of $\{0, 1\}$ with values being the [subnets](../../notes/subnets.md). To generate the [trie](../../notes/trie.md) we build a path from the root for every address in our table and label the end node with that [subnet](../../notes/subnets.md).

> [!example] Prefix-match lookup
> Suppose we have the following table
> ![Prefix Table](../../../static/images/prefix_table.png)
> then we use these strings to construct the following [trie](../../notes/trie.md).
> ![Prefix Trie](../../../static/images/prefix_trie.png)
> we do not include the left branch of this [trie](../../notes/trie.md). We also compress one-way branches for example in P9.

Then we use this [trie](../../notes/trie.md) as the description entails.

# Multibit tries

Whilst unibit [tries](../../notes/trie.md) is very efficient in terms of space they can be slow in terms of memory lookups. For a substring of length $n$ you may need to look up $n$ times. Instead we can increase the length of the "stride" and go for a longer alphabet.
...
Here we pick a length of stride $k$ and then cut our [subnets](../../notes/subnets.md) into multiples of $k$. If a [subnet](../../notes/subnets.md) length is not a multiple of $k$ then we expand it to all the address of the next multiple of $k$ that would be covered b it. If no other longer address clashes with it, we label that address with the [subnet](../../notes/subnets.md) it came from.
...
Then we construct the multibit [trie](../../notes/trie.md) the same way as the unibit [trie](../../notes/trie.md) with the expanded addresses but with the alphabet set to bit strings of length $k$.
...
The optimal stride length can be chosen using [dynamic programming](../../notes/dynamic_programming.md). Though they do not say how.
...
To guarantee more features within a [router](../../notes/router.md) such as quality of service, security and protocol based handling of [packets](../../notes/packets.md) we need to classify packets on more than just their destination [IP](../../notes/internet_protocol_(ip).md).

These further classifications can be based on source [IP address](../../notes/internet_protocol_(ip).md), TCP flags, ect. We refer to this as packet classification. This is deployed by many applications such as:
- Firewalls,
...
If you are going to get a lot of similar [packets](../../notes/packets.md) then caching could be used on top of another search algorithm. This has two problems
...
This involves the sender attaching labels to the header that determine when happens to it. This means that intermediary routers do not need to reclassify packets on a path to the destination. Though this will require security to ensure bad actors can not abuse this. This is deployed in [Multiprotocol label switching (MPLS)](../../notes/multiprotocol_label_switching_(mpls).md) and [DiffServ](../../notes/diffserv.md).

# More [tries](../../notes/trie.md)

Assume we are in a position where we need to use the destination and source [IP address](../../notes/internet_protocol_(ip).md) to determine the rule. The address must match both the [subnets](../../notes/subnets.md) and ties are broken first by the longest matching destination [subnet](../../notes/subnets.md) then by the longest matching source [subnet](../../notes/subnets.md).

A simple way to do this would be to construct the destination [trie](../../notes/trie.md) then at the [leaf](../../notes/leaf_(graph).md) nodes attach source [tries](../../notes/trie.md).

>[!example] Set-Pruning [tries](../../notes/trie.md)
> Suppose we have the following rule table.
> ![Set Pruning Trie Table](../../../static/images/set_pruning_trie_table.png)
> Then we attach the following [tries](../../notes/trie.md) to the destination [trie](../../notes/trie.md).
...
> [!note] This prefers the longest matching source [subnet](../../notes/subnets.md)
> This can be mitigated by switching the order of source and destination when constructing the [trie](../../notes/trie.md).

# Backtracking

We could get around the memory problem by letting each rule only appear in one source [trie](../../notes/trie.md). Then if you don't match in current source [trie](../../notes/trie.md) you backtrack on the destination [trie](../../notes/trie.md) and go to the next source [trie](../../notes/trie.md). In the example above we get the following [trie](../../notes/trie.md).

![Set Pruning Tries Backtrack](../../../static/images/set_pruning_tries_backtrack.png)

Whilst this reduces memory the downside to this approach is the time to compute the answer. If a [packet](../../notes/packets.md) has lots of backtracking you have lots of memory lookups which is slow.

# Grid of [tries](../../notes/trie.md)

We can get the best of both worlds if we precompute where the misses land in the backtracked [trie](../../notes/trie.md). This means we don't need to recalculate the source path we had already walked. In the example above we would get the following.
...
[Head of line (HOL) blocking](../../notes/head_of_line_(hol)_blocking.md)

If we can avoid [HOL blocking](../../notes/head_of_line_(hol)_blocking.md) we achieve a big speedup. In the example above we could do the same message passing but in only 3 iterations. Though for this we sacrifice "fairness".

## Knockout scheme

This uses output queuing to get around [HOL blocking](../../notes/head_of_line_(hol)_blocking.md). If our switching fabric operates $k$ times faster than outputs we can ... This is really not clear what on earth is going on here.

## Parallel iterative matching

Here we let input lines request access for each output line they have a packet for. This goes as follows:
1. Input lines request a route for each output lines they have a [packet](../../notes/packets.md) for.
...
## [FIFO](../../notes/first_in_first_out_(fifo)_queue.md) with tail drop

Traditional schedule approach. Each input link is a [FIFO](../../notes/first_in_first_out_(fifo)_queue.md) queue where if the queue is full any new additions get dropped.

## Need for Quality of Service (QoS)

If you have some QoS guarantees such as maximum delay or a given bandwidth you employ smarter scheduling techniques. Here it is useful to think of network flows - [packets](../../notes/packets.md) that are all following the same route from source to destination. This flow will need to be recognisable from a header.
...
If we could split [packets](../../notes/packets.md) into bits we could employ absolute fairness by switching from flow to flow delivering each [bit](../../notes/bit.md). This is a round robin.
...
Let $R(t)$ be the round number at time $t$. If the router can deliver packets at $\mu$ [bits](../../notes/bit.md) a second and we have $N$ flows then
$$
\frac{dR}{dt} = \frac{\mu}{N}.
$$
For the $i$th packet in a flow of size $p(i)$ (in [bits](../../notes/bit.md)) we will assign $S(i)$ and $F(i)$ to be the start and finish round number for that packet when it arrives in a queue at time $t$. We set $S(i) = \max(R(t), F(i-1))$ and $F(i) = S(i) + p(i)$ i.e. if there are no packets waiting for that flow the start round number should be now - otherwise it should start after the last packet is sent.
