...
- *Bare-metal*: This is where a [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) is installed on top of the hardware.
- *Hosted*: This is where the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) is installed on top of another [OS](../../notes/operating_system_(os).md).

# Bare-metal

In this model the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) is in direct control of the hardware. The main difficulty of the model is integrating different devices - as device manufacturers would need to write a device driver for the hypervisor also. The way most [hypervisors](../../notes/virtual_machine_monitor_(vmm).md) get around this is by hosting a privileged VM that integrates with devices. This also runs other management tasks.
...
[Hardware protection levels](../../notes/hardware_protection_levels.md)

When running a normal [OS](../../notes/operating_system_(os).md) then only 2 rings are really used:
...
- Letting as many operations from the guest [OS](../../notes/operating_system_(os).md) go directly to the hardware as possible. Given it is only using the resources it was allocated in startup.
	- This enables efficiency, to reduce context switching between the OS and the VM
- If the guest [OS](../../notes/operating_system_(os).md) tries to do a privileged operation it does not have sufficient privilege for, this traps to the hypervisor:
	- If the operation is illegal then it terminates that virtual machine.
	- If the operation is legal, the hypervisor carries out the operation for the guest [OS](../../notes/operating_system_(os).md) then returns the [OS](../../notes/operating_system_(os).md) into the same state as if it had carried out the operation.
...
- All guests [OS](../../notes/operating_system_(os).md) expect contiguous physical memory, starting at 0.
- Though there are now 3 different addresses.
	- Virtual: In a process on the guest [OS](../../notes/operating_system_(os).md).
	- Physical: In the guest [OS](../../notes/operating_system_(os).md).
	- Machine: In the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md), i.e. what is actually on the hardware.
- The hardware still has the [MMU](../../notes/memory_management_unit_(mmu).md) and [TLB](../../notes/translation_lookaside_buffer_(tlb).md) to speed up memory mapping, we will want to use this to keep memory look up fast.

There are two options here, however only one is really used.

- Option 1: Multiple page tables:
	- The guest [OS](../../notes/operating_system_(os).md) maintains its own VA -> PA mappings.
	- The [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) maintains the PA -> MA mappings that can be used by the [MMU](../../notes/memory_management_unit_(mmu).md).
	- This is too slow as we would need to do the first mapping all the time in memory.
- Option 2: [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) shadows the page tables.
	- The guest [OS](../../notes/operating_system_(os).md) maintains its page tables which go from VA -> PA.
	- The hypervisor shadows these page tables in the [MMU](../../notes/memory_management_unit_(mmu).md) and converts them to VA -> MA under the hood.
	- Hypervisor maintains consistency by write protecting the [page tables](../../notes/page_table.md) from the [OS](../../notes/operating_system_(os).md) then when the guest [OS](../../notes/operating_system_(os).md) tries to write to them it traps and can apply its mapping to them directly.

In Paravirtualisation as the guest [OS](../../notes/operating_system_(os).md) knows it is virtualised we do not need to maintain two different mappings - instead the guest [OS](../../notes/operating_system_(os).md) registered its page tables to the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) and it takes full control of the [page table](../../notes/page_table.md) management on the [MMU](../../notes/memory_management_unit_(mmu).md). The issue with this is it triggers a trap to the hypervisor on each call to edit a page table, which can be slow. So when changing the [OS](../../notes/operating_system_(os).md) we can make optimizations like batching page table registration to amortise the cost over lots of operations.

In newer platforms these overheads have nearly been completely eliminated.

# Device virtualisation

For [CPU](../../notes/central_processing_unit_(cpu).md) and [RAM](../../notes/random_access_memory_(ram).md) there is low level of diversity in the interfaces. However for generic devices this diversity is massive so virtualising device access is a complicated problem. There are 3 models taken to do this.

## Pass through model

In this model the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) directory passes operations from the guest [OS](../../notes/operating_system_(os).md) to the device. This has the following considerations:
...
- The guest [OS](../../notes/operating_system_(os).md) must have exactly the correct device driver for that device. This makes migration harder and couples applications to machines with exactly that device on it.

![Pass Through Model](../../../static/images/pass_through_model.png)

## Hypervisor direct model

In this model the [hypervisor](../../notes/virtual_machine_monitor_(vmm).md) intercepts any device accesses and translates them into a generic operation. This operation is then processed using the hypervisor and carried out on the device. The response is then converted back to what the guest [OS](../../notes/operating_system_(os).md) expects the response to be.

![Hypervisor Direct Model](../../../static/images/hypervisor_direct_model.png)

This has the following considerations:
- The guest [OS](../../notes/operating_system_(os).md) is completely decoupled from the physical device.
...
This model is limited to only paravirtualised [hypervisors](../../notes/virtual_machine_monitor_(vmm).md) and requires them to be running a service VM. This uses two different device drivers:
...
Due to the benefits of virtualization the industry has been working to make [virtualization](../../notes/virtualization.md) easier:
...
- Extended page tables and tagged [TLB](../../notes/translation_lookaside_buffer_(tlb).md) with VM ids so clearing the cache can be made more efficient.
