...
week: 11
---
# Model of a [IO device](../../notes/input_output_(io).md)

Whilst an [IO](../../notes/input_output_(io).md) device can come in many forms we model them similarly.
...
Devices are mainly connected to the [CPU](../../notes/central_processing_unit_(cpu).md) through the [Peripheral component interconnect (PCI)](../../notes/peripheral_component_interconnect_(pci).md) or [PCI Express (PCIe)](../../notes/pci_express_(pcie).md). Though depending on the component it may be connected to that via another bus as well.

![Inter Connect To Cpu](../../../static/images/inter_connect_to_cpu.png)

# Device driver

[Device driver](../../notes/device_driver.md)
...
[Pseudo devices](../../notes/pseudo_devices.md)

There are two methods in which the [CPU](../../notes/central_processing_unit_(cpu).md) communicates with devices.

- *Memory-mapped I/O*: On boot some part of the main memory address space is registered for use by a device. The [CPU](../../notes/central_processing_unit_(cpu).md) can then write or read to these addresses to interact with the device.
...
There are two ways for the device to communicate with the [CPU](../../notes/central_processing_unit_(cpu).md):

- *Interrupts*: The device can generate interrupts that the [OS](../../notes/operating_system_(os).md) will need to handle. This gives the device the best access to the [OS](../../notes/operating_system_(os).md) but can be inefficient in terms of caching if the input is not needed.
- *Polling*: The [CPU](../../notes/central_processing_unit_(cpu).md) can poll the device for information by checking its status registers. This means the [CPU](../../notes/central_processing_unit_(cpu).md) can only get the information when it needs it - however it can generate delay or overhead if the [CPU](../../notes/central_processing_unit_(cpu).md) needs to repeatedly poll the device.

[Programmed IO (PIO)](../../notes/programmed_io_(pio).md)

[direct memory access](../../notes/direct_memory_access_(dma).md)

# Device access

Device access is an intensive action as we need to parse through many different layers and processes. Thus why [IO](../../notes/input_output_(io).md) is considered slow.

![Io Access](../../../static/images/IO_access.png)

Some devices support [OS](../../notes/operating_system_(os).md) bypass. This is where the [OS](../../notes/operating_system_(os).md) configures the device and the process to communicate directly through the [processes](../../notes/process.md) virtual address space with limited functionality. Though the device has some requirements for this:

- The device producer must offer a user-level library to enable this.
- The device must have sufficient registers for the process to have access but also for the [OS](../../notes/operating_system_(os).md) to have more fine-grained control over the device.
- If the device supports multiple accesses then it must be sufficiently complex to deduplex the requests and send them to the correct device (i.e. looking at [port](../../notes/port.md) numbers).

However, if the device does support this it can increase access speed dramatically.

When accessing [IO](../../notes/input_output_(io).md) devices this can either happen synchronously or [async](../../notes/asynchronous_programming.md). Normally when issuing commands to a device the process needs to wait for a response from the device. This can either block the process and it gets moved to a wait queue or in a non-blocking way. When using async the process can either poll or get interrupted to get output from the device.

![Async Vs Sync Device Access](../../../static/images/async_vs_sync_device_access.png)

# Block device access

When interacting with block devices, user programs actually interact with files that are on the computers filesystem. Files are considered one 'logical' storage unit. The [OS](../../notes/operating_system_(os).md) will specify how to support writing to files through an API - normally the [POSIX](../../notes/portable_operating_system_interface_(posix).md) API.

![Block Device Stack](../../../static/images/block_device_stack.png)

As well as having a device driver for particular devices a further abstraction is placed on top of these to standardize the interaction - this is the generic block layer. This is an [OS](../../notes/operating_system_(os).md) standard for interfacing with different block devices.

# Virtual file system

The [OS](../../notes/operating_system_(os).md) abstracts the concepts of block devices away from the user through a virtual file system. It uses this to combine potentially different types of block devices into one directory structure which is easy for humans to grasp.
