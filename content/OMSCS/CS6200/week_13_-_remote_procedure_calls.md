...
The goal of [RPC](../../notes/remote_procedure_calls_(prc).md) is that the server and client do not need to be programmed together or even written in the same language. To achieve this we need some language agnostic way to define the methods that can be called and the messages that are needed by them. This is where an [Interface definition language (IDL)](../../notes/interface_definition_language_(idl).md) comes in.

[Interface definition language (IDL)](../../notes/interface_definition_language_(idl).md)

An important aspect to any [IDL](../../notes/interface_definition_language_(idl).md) is it includes a version number. This allows for easy upgrades to any interface you have defined.
...
Both these will not need to be written by the developer - instead they will be built into how the [RPC](../../notes/remote_procedure_calls_(prc).md) framework is built.
...
[RPC](../../notes/remote_procedure_calls_(prc).md) frameworks implement error handling however, due to the framework operating over a network it sometimes can not know the cause of the issue. Therefore it offers a best guess at issues within this framework.

# SunRPC

This is an [RPC](../../notes/remote_procedure_calls_(prc).md) framework developed by Sun in the 80s (bought by Oracle) for unix. They made the following design choices:

- Hosts a registry on each machine,
- It uses the XDR [IDL](../../notes/interface_definition_language_(idl).md).
...
- Transport layer: Whether it uses [TCP](../../notes/transmission_control_protocol_(tcp).md) [UDP](../../notes/user_datagram_protocol_(udp).md) or [IPC](../../notes/inter-process_communication_(ipc).md).
