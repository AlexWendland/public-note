...
This technology is become more relevant in data centers since the development of [Remote direct memory access (RDMA)](../../notes/remote_direct_memory_access_(rdma).md).
...
These both happened as soon as a write happened which may course too much overhead in distributed settings. In [distributed file system](../../notes/distributed_file_system_(dfs).md) we saw two different methods.
...
- We can us other [MMU](../../notes/memory_management_unit_(mmu).md) state such as dirty pages to assist with this.
