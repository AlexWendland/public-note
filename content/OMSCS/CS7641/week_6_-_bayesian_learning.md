...
To start this lecture lets remind ourselves of the definition of [conditional probability](../../notes/conditional_probability.md).

[Conditional probability](../../notes/conditional_probability.md)

As a simple corollary we get [Bayes rule](../../notes/bayes_rule.md).

[Bayes rule](../../notes/bayes_rule.md)

>[!question]
>Suppose there is a illness effects $0.8\%$ of the population $\mathbb{P}[I] = 0.008$. We have a test that has a [true positive](../../notes/result_types.md) chance of $98\%$ (i.e. $\mathbb{P}[+ve | I] = 0.98$) and a [true negative](../../notes/result_types.md) result of $97\%$ (i.e. $\mathbb{P}[-ve \vert \lnot I] = 0.97$). Given you have a positive test result are you more likely to have the illness or not?

Here we apply [Bayes rule](../../notes/bayes_rule.md).
...
Suppose $h \in H$ is a hypothesis belonging to our [hypothesis space](../../notes/modelling_paradigm.md) and we have $D$ data. Then to see the probability our hypothesis is given the data $\mathbb{P}[h \vert D]$ we can use [Bayes rule](../../notes/bayes_rule.md) to reduce it to things we can calculate
$$
...
When we are training our model on [training data](../../notes/training_data.md) $T$, we are trying to find
$$
\mbox{arg}\max_{h \in H} \mathbb{P}[h \vert T].
$$
Though for each $h \in H$ we have the same $\mathbb{P}[D]$ as this does not depend on $h$. So we might as well remove this from our calculation - here we get the [maximum a posteriori probability](../../notes/maximum_a_posteriori_probability_estimate_(map).md).

[Maximum a posteriori probability estimate (MAP)](../../notes/maximum_a_posteriori_probability_estimate_(map).md)

Sometimes we will have no prior preference on the [hypothesis space](../../notes/modelling_paradigm.md) in this case we might as well assume it is uniform and remove it from our calculations - here we get the [maximum likelihood estimation](../../notes/maximum_likelihood_estimation_(mle).md).

[Maximum likelihood estimation](../../notes/maximum_likelihood_estimation_(mle).md)

Though to actually calculate these would be very hard - as the [hypothesis space](../../notes/modelling_paradigm.md) might be very large.

# Noise free data

Suppose:
- There is some target $c : A \rightarrow B$.
- We have some [irreducible error](../../notes/irreducible_error.md) free (noise free) [training data](../../notes/training_data.md) $T$, so for $(a,b) \in T$ we have $b = c(a)$.
- We have a finite [hypothesis space](../../notes/modelling_paradigm.md) $H$ which contains the target $c \in H$.
- We have no prior preference on the [hypothesis space](../../notes/modelling_paradigm.md) $H$.
- Each hypothesis is an [independent event](../../notes/independent_events.md).

Now we are use [Bayes rule](../../notes/bayes_rule.md) to calculate $\mathbb{P}(h \vert T)$ for each $h \in H$.
...
however this describes the [version space](../../notes/version_space.md) for $H$ with $T$, $VS_H(T)$, so
$$\mathbb{P}[T \vert h] = \mathbb{I}[h \in VS_H(T)].$$
- As each hypothesis is an [independent event](../../notes/independent_events.md) we have
...
- We have some [i.i.d.](../../notes/independent_identically_distributed_samples.md) [normally distributed](../../notes/normal_distribution.md) noise values $\epsilon \sim N(0,\sigma^2)$ for each of our [training data](../../notes/training_data.md) $t \in T$.
- The [training data](../../notes/training_data.md) $T$ is such that $(a_i,b_i) \in T$ we have $b_i = c(a_i) + \epsilon_i$.
...
[Length of a probability](../../notes/length_of_a_probability.md)
...
[Bayeses optimal classifier](../../notes/bayeses_optimal_classifier.md)
