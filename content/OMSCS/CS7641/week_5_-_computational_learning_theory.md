...
[Computational learning theory](../../notes/computational_learning_theory.md) is fundamentally about 3 things:
...
3. Complexity of the hypothesis class.
	1. How complex is the representation - [modelling paradigm](../../notes/modelling_paradigm.md)
4. Accuracy to which the target concept is approximated.
	1. Within some $\epsilon$.
5. Manner in which the examples are presented.
	1. Batch or online.
6. Manner in which the [training data](../../notes/training_data.md) is collected.
...
20 Questions is a game where there is a set of possible answers $A$ and a correct answer $c \in A$. There is a player (questioner) who wants to find $c \in A$ and a questionee who knows $c \in A$. The player then asks questions [boolean](../../notes/boolean_variable.md) questions to questionee who answers True or false about item $c \in A$.

> [!note] 20 Questions
> In the traditional version of the game you have until 20 questions to guess the answer, thus the name!

There are different set ups for this game - but the point is to demonstrate the power of how [Training data](../../notes/training_data.md) is collected.
...
[Run time complexity](../../notes/run_time_complexity.md)

However there are some domain specific definitions as well.

[Sample complexity](../../notes/sample_complexity.md)

Touching upon the example above.

[Mistake bound](../../notes/mistake_bound.md)

Lets fix terminology for the next section. Suppose we are in the [modelling framework](../../notes/modelling_framework.md) with some training data $T$.
- Given some [hypothesis space](../../notes/modelling_paradigm.md) $H$ the "true hypothesis" is $c \in H$ (i.e. for all $(a,b) \in T$ $c(a) = b$).
- The current candidate hypothesis will be $h \in H$.
- A [consistent learner](../../notes/consistent_learner.md) $h$ is one where $h(a) = c(a)$ for all $(a,b) \in T$.
- The [version space](../../notes/version_space.md) VS(T) = \{$h \ \vert$  $h$ is a consistent learner with respect to $T$\}.

## Pac learning

There are two concepts of error when evaluating a hypothesis.

[Training error](../../notes/training_error.md)

[True error](../../notes/true_error.md)

Using this we can define what we call "probably approximately correct" (PAC) learner.

[PAC learner](../../notes/probably_approximately_correct_learnable_(pac).md)
...
This is useful concept for finding the number of samples we need to take to us to give us a [PAC learner](../../notes/probably_approximately_correct_learnable_(pac).md).
...
A weakness of [Haussler Theorem](../../notes/haussler_theorem.md) is that it doesn't handle infinite dimensional hypothesis spaces.
