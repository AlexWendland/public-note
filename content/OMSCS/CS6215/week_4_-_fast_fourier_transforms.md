...
This can also be summaries using [Euler's formula](../../notes/euler's_formula.md) by saying
...
So to compute it for points $x_0, \ldots, x_{n-1}$ we do this via [matrices](../../notes/matrix.md) using the following form
...
when $x_j = \omega_n^j$ we denote this as $A = M_n(\omega_n) a$. So computing the inverse of the [FFT](../../notes/fast_fourier_transform.md) is simply calculating $M_n(\omega_n)^{-1} A = a$,
...
\end{align*}$$
Which splits into cases if $j = k$ then $\omega_n^{j-k} = 1$ and we have this sum is $n$. Whereas if $j \not = k$ from the [sum of roots of unity](../../notes/sum_of_roots_of_unity.md) we have this sum is 0. Therefore
$$ M_n(\omega_n^{-1}) M_n(\omega_n) = n I_n$$
giving us the desired result.

$\square$

Now the problem once again is of the form $n a = M_n(\omega_n^{n-1}) A$ we can use the [FFT](../../notes/fast_fourier_transform.md) algorithm detailed before to solve the inverse of the [FFT](../../notes/fast_fourier_transform.md).
