...
If you were to just do those computations, it would take you $O(n^2)$ time as you need to do $O(n)$ steps computing up to $O(n)$ terms for each step. However, this can be improved to $O(n\log(n))$.
...
> [!lemma] Uniqueness of polynomials
> A polynomial of degree $n-1$ is uniquely determined by its values at any $n$ distinct points.
...
Note here we used $2n$ points. The plan of attack will be to convert polynomials from coefficients into values using [fast Fourier transforms](../../notes/fast_fourier_transform.md), multiply them, then transform back using the same technique.
...
- $a_{2j}x_i^{2j} = a_{2j}x_{i+n}^{2j}$ and
- $a_{2j+1}x_i^{2j+1} = - a_{2j+1}x_{i+n}^{2j+1}$.
So it is meaningful to split up the polynomial into odd and even terms and define
$$A_{even}(y) = \sum_{i=0}^{\lfloor n/2 \rfloor - 1} a_{2i} y^i, \mbox{ and } A_{odd}(y) = \sum_{i=0}^{\lfloor n/2 \rfloor - 1} a_{2i + 1} y^i.$$
so we have $A(x) = A_{even}(x^2) + x A_{odd}(x^2)$. This will begin our divide and conquer approach.

>[!note] Reduction in degree only
...
Note that
$$A(x_i) = A_{even}(x_i^2) + x_i A_{odd}(x_i^2), \mbox{ and } A(x_{n+i}) = A(-x_i) = A_{even}(x_i^2) - x_i A_{odd}(x_i^2)$$
So given $A_{even}(y_i)$ and $A_{odd}(y_i)$ for $y_i = x_i^2$ where $1 \leq i \leq n$, we get $A(x_j)$ for $1 \leq j \leq 2n$.
...
To achieve this we will need to look at the polynomials in the [complex numbers](../../notes/complex_numbers.md), which we denote as $\mathbb{C}$.

To work with complex numbers remember we set $i = \sqrt{-1}$ and think of them as sums $a + bi$.

These can be represented as $(a,b)$ or we can represent them using [Polar Coordinates](../../notes/polar_coordinates.md) using $r$ and $\theta$, where
$$(a,b) = (r \cos(\theta), r \sin(\theta)).$$
This can also be summarised using [Euler's formula](../../notes/euler's_formula.md) by saying
$$a + bi = r(\cos(\theta) + i \sin(\theta)) = r e^{i\theta}.$$
# Roots of unity

The $k$-th [roots of unity](../../notes/roots_of_unity.md) are those $z \in \mathbb{C}$ such that $z^k = 1$. For $k=2$ these are simply $1, -1$; however, for $k=4$ they are $1, -1, i, -i$.

More generally, these are $e^{\frac{2\pi j}{k} i}$ for $0 \leq j < k$. In the notation of this course, set $\omega_k = e^{\frac{2\pi}{k} i}$ and express the roots of unity as $\omega_k^j$ for $0 \leq j < k$.

> [!note] Properties of use
> When $k = 2n$ is even note that $\omega_k^2 = \omega_n$ as well as $\omega_k^j = - \omega_k^{j + n}$ as $\omega_k^n = -1$.

This means that even powers of the [roots of unity](../../notes/roots_of_unity.md) are perfect choices for our $x_i$ in the [fast Fourier transform](../../notes/fast_fourier_transform.md) algorithm.

# Pseudocode for FFT

``` pseudocode
FFT(a, w):
	input: coefficients a = (a_0, a_1, ..., a_{n-1}) for polynomial A(x) where
		n is a power of 2 and w is an n-th root of unity.
	output: A(w^0), A(w), A(w^2), ..., A(w^{n-1})
	if n = 1
		return A(1)
	Let A_even = (a_0, a_2, ..., a_{n-2}) and A_odd = (a_1, a_3, ..., a_{n-1})
...
If $T(n)$ is the running time of our algorithm, let us analyse this. The steps to split up the polynomial and glue the answers back together each take $O(n)$ time. Then we make two recursive calls that both take $T(n/2)$, so the running time is $T(n) = 2T(n/2) + O(n)$ as we stated above.

> [!question] How do we go backwards?

# Linear algebra of FFT

The [linear algebra](../../notes/linear_algebra.md) of what we are doing here is important to how we compute the inverse.

For a point $x_j$, we have
$$A(x_j) = \sum_{i=0}^{n-1} a_i x_j^i = (1, x_j, \ldots, x_j^{n-1}) \cdot (a_0, a_1, \ldots, a_{n-1}).$$
To compute this for points $x_0, \ldots, x_{n-1}$, we do this via [matrices](../../notes/matrix.md) using the following form
...
When $x_j = \omega_n^j$, we denote this as $A = M_n(\omega_n) a$. Computing the inverse of the [FFT](../../notes/fast_fourier_transform.md) is simply calculating $M_n(\omega_n)^{-1} A = a$.

>[!lemma] Inverse of the Vandermonde matrix
...
Now the problem is again of the form $n a = M_n(\omega_n^{n-1}) A$. We can use the [FFT](../../notes/fast_fourier_transform.md) algorithm detailed above to compute the inverse of the [FFT](../../notes/fast_fourier_transform.md).

# Pseudocode for Polynomial Multiply

``` pseudocode
PolynomialMultiply(a, b):
	input: coefficients a = (a_0, a_1, ..., a_{n-1}) for polynomial A(x) and
		coefficients b = (b_0, b_1, ..., b_{n-1}) for polynomial B(x)
	output: coefficients c = (c_0, c_1, ..., c_{2n-2}) for polynomial C(x) =
		A(x) B(x)
	Let w_{2n} be a primitive 2n-th root of unity
	(r_0, r_1, ..., r_{2n - 1}) = FFT(a, w_{2n})
	(s_0, s_1, ..., s_{2n - 1}) = FFT(b, w_{2n})
	for j = 0 to 2n-1
		t_j = r_j * s_j
	(c_0, c_1, ..., c_{2n-1}) = (1/2n) FFT(t, w_{2n}^{-1})
```

The running time of this algorithm is $O(2n\log(2n)) = O(n\log(n))$ for the three FFT steps, plus $O(n)$ for calculating the entries for $t_j$, giving a total running time of $O(n\log(n))$.
