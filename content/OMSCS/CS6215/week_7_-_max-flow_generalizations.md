...
We would like to turn the [flow network](../../notes/flow_network.md) with demands into a problem of just the [max flow](../../notes/max_flow_problem.md).
...
We will define a new [flow network](../../notes/flow_network.md) $(G' = (V',E'), c', s', t')$ to do this. First lets define the graph $G'$ with $V' = V \cup \{s', t'\}$ and
...
Then a [flow](../../notes/flow.md) $f'$ on $(G', c', s', t')$ will be converted into a flow on $(G,c,s,t)$ by defining $f(e) = f'(e) + d(e)$ for $e \in E$.

However, this will only be a valid [flow](../../notes/flow.md) on $(G,c,s,t)$ if the value of $capacity(f') = \sum_{e \in E} d(e)$ (the maximum capacity on $(G', c', s', t')$ ). Such a [flow](../../notes/flow.md) $f'$ will be called a saturating [flow](../../notes/flow.md).
...
We run [Ford-Fulkerson Algorithm](../../notes/ford-fulkerson_algorithm.md) or [Edmonds-Karp algorithm](../../notes/edmonds-karp_algorithm.md) but instead of starting from a zero flow we instead start from $f$.

Also instead of defining the usual [residual network](../../notes/residual_network_(flow).md) instead we define a limited version
