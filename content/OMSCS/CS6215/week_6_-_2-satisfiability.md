...
[Statement](../../notes/satisfiability_problem_(sat_problem).md#statement)
...
[Statement](../../notes/k-satisfiability_problem_(k-sat_problem).md#statement)

[Theory](../../notes/k-satisfiability_problem_(k-sat_problem).md#theory)
...
We can now assume our formula has all clauses of size 2, it will have $n$ variables $x_i$ and $m$ clauses $c_i = \alpha_i \lor \beta_i$ where $\alpha_i, \beta_i \in \{x_i, \overline{x_i} \vert 1 \leq i \leq n\}$. We want to form a [directed graph](../../notes/directed_graph.md) $D = (V,E)$ to represent this problem.
...
>[!important] Lemma
>If there is a [path](../../notes/path_(graph).md) between $a$ and $b$ in $G$ then if $a$ is true in a assignment to the variables that satisfies $f$ then $b$ must be true also.

### Proof

Prove this by induction on the length of [path](../../notes/path_(graph).md).

Suppose the [path](../../notes/path_(graph).md) is of length 1, i.e. $(a, b) \in E$. Therefore there is a clause $\overline{a} \lor b$ or $b \lor \overline{a}$, so if $a$ is true then $b$ needs to be true.

Suppose it is true for a path of length less than $k$.

Suppose we have a [path](../../notes/path_(graph).md) of length $k>1$ $p (c,b)$ where $p$ is a [path](../../notes/path_(graph).md) from $a$ to $c$ of length $k-1$.
...
> [!lemma] Lemma
> If for some $i$, $x_i$ and $\overline{x_i}$ are in the same [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md) then $f$ is not satisfiable.
...
>[!important] Lemma
>If for all $i$, $x_i$ and $\overline{x_i}$ are in different [strongly connected components](../../notes/strongly_connected_components_(directed_graphs).md) then $f$ is satisfiable.
...
This means in $G$ it is meaningful to talk about complement paths. i.e. if $p_{a,b}$ is a [path](../../notes/path_(graph).md) in $G$ from $a$ to $b$ then $\overline{p_{a,b}}$ is the complement path from $\overline{b}$ to $\overline{a}$ that we will refer to as $p_{\overline{b}, \overline{a}} := \overline{p_{a,b}}$.

This actually gives us quite a neat corollary.

>[!important] Corollary
>If $S$ is a [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md) in $G$ then $\overline{S} = \{\overline{s} \vert s \in S\}$ is a [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md) in $G$.

### Proof

For any two $s_1, s_2 \in S$ there exists a path $p_{1,2}$ and $p_{2,1}$ connecting $s_1$ to $s_2$ and $s_2$ to $s_1$.

By the Lemma above $\overline{p_{1,2}}$ and $\overline{p_{2,1}}$ connects $\overline{s_2}$ to $\overline{s_1}$ and $\overline{s_1}$ to $\overline{s_2}$. Showing that $\overline{s_1}$ and $\overline{s_2}$ are [strongly connected](../../notes/strongly_connected_(directed_graphs).md) to one another.

Equally if $\overline{t} \in V$ is [strongly connected](../../notes/strongly_connected_(directed_graphs).md) to $\overline{s} \in \overline{S}$ via paths $\overline{p_{t,s}}$ and $\overline{p_{s,t}}$ then $p_{t,s}$ and $p_{s,t}$ strongly connect $t$ to $s$ giving $t \in S$.

Therefore $\overline{S}$ is a [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md) of $G$.
...
Let $S$ be a [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md) in the [strongly connected component graph](../../notes/strongly_connected_component_graph_(directed_graph).md).

From the Corollary above we know $\overline{S}$ is a [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md).
...
Find the [strongly connected component graph](../../notes/strongly_connected_component_graph_(directed_graph).md) of $G$, $SCC(G)$.

We can pair the [strongly connected components](../../notes/strongly_connected_components_(directed_graphs).md) $S$ with $\overline{S}$. From the assumption of the lemma $S \not = \overline{S}$ for any [strongly connected component](../../notes/strongly_connected_components_(directed_graphs).md).

To satisfy $f$ we need to find a True/False allocation to the [strongly connected components](../../notes/strongly_connected_components_(directed_graphs).md) such that:
...
[2-SAT algorithm using SCC](../../notes/2-sat_algorithm_using_scc.md#2-sat-algorithm-using-scc)
...
[Run time](../../notes/2-sat_algorithm_using_scc.md#run-time)
