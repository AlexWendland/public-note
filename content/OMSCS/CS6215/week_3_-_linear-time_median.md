...
> Given an unsorted list $A = [a_1, \ldots, a_n]$ of $n$ numbers - you want to find the [median](../../notes/median.md) ($\lceil \frac{n}{2} \rceil$) element.

> [!note] Not usual [Median](../../notes/median.md) definition
...
# Using [Merge sort](../../notes/merge_sort.md)

A basic algorithm would be to use [Merge sort](../../notes/merge_sort.md) on $A$ then output the $k$-th element of that list which would take $O(n\log(n))$. Though is it possible to do this without sorting $A$?
...
$$ T(n) = T(\frac{3n}{4}) + O(n),$$
which by [Masters theorem](../../notes/masters_theorem.md) gives $T(n) = \Theta(n)$.
