...
> We allocate IDs to nodes if they are the next highest node in the circle (looping back around once we reach the top). Then when a node exits, all the IDs just overflow to the next node.
> ![Consistent Hashing Exit](../../static/images/consistent_hashing_exit.png)
