...
---
> [!lemma] Lemma
> Suppose we have a [directed graph](directed_graph.md) $G = (V,E)$ and $post: V \rightarrow \mathbb{N}$ be the result of running the [DFS algorithm to find connected components in an undirected graph](dfs_to_find_connected_components_in_an_undirected_graph.md). Then the vertex with the largest post order number lies in a source [strongly connected component](strongly_connected_components_(directed_graphs).md) in the [strongly connected component graph](strongly_connected_component_graph_(directed_graph).md).
