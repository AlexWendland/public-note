...
This takes $O(1)$ as we don't need to change the solution. Therefore this is [polynomial time](polynomial_time.md).
...
Therefore this is a valid [many-one reduction](many-one_reduction_(problem).md) and we have that [Knapsack-search](knapsack-search_(without_replacement).md) is [NP-complete](np-complete.md).
