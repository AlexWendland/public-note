...
$$
To start the algorithm we pick $k$ random points in our [training data](training_data.md) $T$. We set these to be the current centres $center^0_i$ for $1 \leq i \leq k$. For iteration $t \in \mathbb{N}$ assume we have centres $center_i^{t-1} \in A$ (they don't need to be points in $T$). Now we group the training data
$$
...
Let the configurations be the [partition](partition_(set).md) with the centres $(C_i, center_i)_{1 \leq i \leq k}$. Then we can score each configuration as
$$
...
Then our algorithm is identical to the [Hill climbing](hill_climbing.md) algorithm in this set up.

>[!note] Double hop
> This argument only works when you think of one iteration of our algorithm being 2 steps in the [Hill climbing](hill_climbing.md). First you update the centres, one hop, then you update the partition, two hop.
...
Given this is the same as [Hill climbing](hill_climbing.md) we can get stuck in a local optimum which is not a global optimum. There are two ways to get around this:
