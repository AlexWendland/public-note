...
The solution provides $C$ a max [clique](clique_(graph).md). If $|C| \geq g$, then return $C$, otherwise return no. This step takes $O(|V|)$ so it is polynomial in the input size.
...
If the [graph](graph.md) $G$ has a clique of size at least $g$, then its max [clique](clique_(graph).md) is of size $\geq g$ by definition.
