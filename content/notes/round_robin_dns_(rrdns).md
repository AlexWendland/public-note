...
type: definition
---
>[!definition] Round robin DNS (RRDNS)
>Round Robin [DNS](domain_name_system_(dns).md) is a technique used by large websites to manage and distribute incoming traffic across multiple servers at a single location. When a DNS request is made, RRDNS responds with a list of IP addresses (A records) for the requested domain. These addresses are rotated in sequence with each request, ensuring that no single server is overloaded with too many requests.
>
