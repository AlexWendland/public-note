...
The solution provides $I$ a max [independent set](independent_set_(graph).md). If $\vert I \vert \geq g$ then return true, otherwise return false. This step takes $O(\vert V \vert)$ so it is polynomial in the input size.

If the [graph](graph.md) $G$ has an [independent set](independent_set_(graph).md) of size at least $g$ then its max independent set is of size $\geq g$, so we return true in this case.

If the [graph](graph.md) $G$ does not have an [independent set](independent_set_(graph).md) of size at least $g$ then its max [independent set](independent_set_(graph).md) is of size $< g$, so we return false in this case.
