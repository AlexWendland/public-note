...
Suppose $G$ has some cycle $x_1, x_2, \ldots x_n$ where $x_i \in V$ and $(x_i, x_{i+1}), (x_n, x_1) \in E$. Then some vertex $x_k$ must have been discovered first by $A$.

All other vertices in the cycle must be below $x_k$ and contained in its branch. Therefore $x_{i-1}$ is contained in its branch with edge $(x_{i-1}, x_i)$ giving the desired [back edge](dfs_tree_(algorithm).md).

### $\Leftarrow$

Suppose we have some back edge $(a, b) \in E$. By the definition of a back edge, $(a,b) \not \in E'$ but $a$ and $b$ are in the same branch in $T$.
