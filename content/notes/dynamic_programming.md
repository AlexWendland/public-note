...
(If this feels a lot like [Induction](induction.md), that is because this is essentially a programming equivalent.)

> [!example] Longest [increasing](increasing_sequence.md) [subsequence](subsequence.md) (LIS)
> Suppose we are provided with a [sequence](sequence.md) of $n$ numbers $a_1, a_2, \ldots, a_n$ and we want to find the length of the longest [increasing](increasing_sequence.md) [subsequence](subsequence.md) in $\{a_i\}_{i=1}^n$.
>
> This can be done using [dynamic programming](dynamic_programming.md) with the following steps:
>
