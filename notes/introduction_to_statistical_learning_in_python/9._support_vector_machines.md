...
name: 9. Support Vector Machines
tags: []
type: lecture
---
...
[Hyperplane](../general/hyperplane.md)

Another way to define a hyperplane is to use an equation. This is to take an orthogonal vector $o$ to the subspace spanned by $v_1, v_2, \ldots, v_n$ perform the [Dot product](../general/dot_product.md) with that vector and set it equal to the dot product of $o \cdot x$. Intuitively this is the "distance" away from the hyperplane that point is.
...
Suppose we have $k$ points of [training data](../general/training_data.md) with classification labels $1, -1$. This will be represented by $(x^j, y^j)$ for $1 \leq j \leq k-1$, where $x^j \in \mathbb{R}^n$ and $y^j \in \{1, -1\}$. Lets assume the data is linearly separable so there exists $w_i, \theta \in \mathbb{R}$ such that
$$
...
$$
(note I replaced $-\theta$ with $\theta$ to make life easier) relates to how far the point $x^j$ is away from the [hyperplane](../general/hyperplane.md) $P$. We call this the margin for that point and ideally we would want to maximise the minimum margin. This will make the least assumptions about our data and reduce the chances of [overfitting](../general/overfitting.md).

A [hyperplane](../general/hyperplane.md) that achieves this is called a *maximal margin classifier*. I.e. it is looking for the max $M$ such that
$$
...
This kind of problem is called [Quadratic programming problem](../general/quadratic_programming_problem.md) which is not touched by the book.

## Aside (neural networks)

This is essentially the same as a neural network at this point!

[perceptron](../general/perceptron_(neural_network).md)

[binary step](../general/binary_step.md)

## Non-seperating case

Your data might not be [Linearly separable](../general/linearly_separable.md) or moreover the separator that does cut your data in half might be massively over fitting.
