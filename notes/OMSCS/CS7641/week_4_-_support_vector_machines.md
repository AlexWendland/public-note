...
We are going to revisit the idea of [linearly separable](../../general/linearly_separable.md) points in the plane.

[Linearly separable](../../general/linearly_separable.md)

Suppose we have [training data](../../general/training_data.md) $(x^t, y^t) \in \mathbb{R}^n \times \{1,-1\}$ for $t \in T$ such that $X_1 = \{x^t \vert y^t = 1\}$ and $X_2 = \{x^t \vert y^t = -1\}$ are linearly separable. This means that there exists some $w \in \mathbb{R}^n$ and $b \in \mathbb{R}$ such that
$$
y_t ( x^t \cdot w + b ) = y_t \left ( \sum_{i=1}^n x^t_i w_i + b \right ) \geq 0
$$
for every $t \in T$.

[Dot product](../../general/dot_product.md)
...
When you have some points that are [linearly separable](../../general/linearly_separable.md) there might be many choices of $w$ and $b$. Though which one is the best? And what does the best mean?

Ideally we would want to choose a line that:
- [Linearly separates](../../general/linearly_separable.md) the points, whilst
- maximising the minimum distance between the points and the line.

The intuition as to why we would want this is by making our separating line the furthest away from the points it can be whilst still separating them means we are trying to avoid [overfitting](../../general/overfitting.md) the best we can. The line is as far away from our training data as it possibly can be.
...
[Margin for a linear separator](../../general/margin_for_a_linear_separator.md)
...
$$
The distance between the [hyperplanes](../../general/hyperplane.md) defined by $H_1 = (x_1, w)$ and $H_2 = (x_2, w)$ is given by
$$
...
$$
Therefore if we make the [linearly separable](../../general/linearly_separable.md) constraint that
$$
...
For [linearly separable](../../general/linearly_separable.md) $(x^t, y^t) \in \mathbb{R}^n \times \{-1,1\}$ find $w \in \mathbb{R}^n$ and $b \in \mathbb{R}$ where $\max_{w, b} \frac{1}{\vert \vert w \vert \vert}$ such that
$$
...
>are called support vectors. As they constrain the [hyperplane](../../general/hyperplane.md) $(w, b)$.
## Back to reality

Our training data is nearly never [linearly separable](../../general/linearly_separable.md) so how can we adapt what we have to handle this?
...
This is apparently a [quadratic programming problem](../../general/quadratic_programming_problem.md) and can be transformed to the  "Dual Optimization Problem" which is as follows
...
$$
The form should remind you of [Boosting](../../general/boosting.md) where we take an average of lots of different views on the data. However in reality lots of these $\alpha_t = 0$ if they are not the support vectors closest to the line. So it might be more correct to think of this as close to [KNN](../../general/k-nearest_neighbour.md).
...
[Kernel trick](../../general/kernel_trick.md)

There are some popular [kernels](../../general/kernel_trick.md) to try.

[Polynomial kernel (SVMs)](../../general/polynomial_kernel_(svms).md)

[Gaussian kernel (SVM)](../../general/gaussian_kernel_(svm).md)

[Sigmoid kernel (SVM)](../../general/sigmoid_kernel_(svm).md)

## Support Vector Machines

We are now going to take the idea of the [kernel trick](../../general/kernel_trick.md) where we embed our feature space $A$ using $\Phi$ into a space in which our dataset is closer to [linearly separable](../../general/linearly_separable.md). This turns our optimisation problem from before into
...
$$
However, we only have use $\Phi$ on two vectors who immediately get the [dot product](../../general/dot_product.md) applied to them - so we can replace this with the kernel instead.

[SVM](../../general/support_vector_machines_(svm).md)

## Why doesn't [Boosting](../../general/boosting.md) have as many problems with [Overfitting](../../general/overfitting.md)

In practice when using [Boosting](../../general/boosting.md) we tend to find the training error and testing error follow each other, rather than separate as in the case with [overfitting](../../general/overfitting.md). This is connected to [SVMs](../../general/support_vector_machines_(svm).md) by the concept of [margins](../../general/margin_for_a_linear_separator.md). When we talked about [SVMs](../../general/support_vector_machines_(svm).md) we stated that a larger [margin](../../general/margin_for_a_linear_separator.md) was better as it reduced the [overfitting](../../general/overfitting.md). The [margin](../../general/margin_for_a_linear_separator.md) is analogous to the confidence level we have in our model.

In relation to [Boosting](../../general/boosting.md) we can think of the algorithm as a complicated way of projecting $\Phi: A \rightarrow \mathbb{R}$ before taking the sign of the value to determine our prediction. As we train for a longer period of time we separate data points more and more - increasing the [margin](../../general/margin_for_a_linear_separator.md) and thus our confidence in the outcomes. This counter intuitively reduces [overfitting](../../general/overfitting.md) in a lot of cases.

This does not hold for all [Boosting](../../general/boosting.md) for example if we use this with [Neural networks](../../general/neural_network.md) it will still be liable to [overfit](../../general/overfitting.md).
