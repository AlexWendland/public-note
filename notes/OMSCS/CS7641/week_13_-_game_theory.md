...
\end{array}
$$
to read this table, if the first player at the top picks action $a$ and the second player on the rows picks $x$ then player one gets 1 point whereas player two gets -1 point. This is the [zero-sum](../../general/zero-sum_game.md) aspect to the game. (Note the game could be non-deterministic but we just take [expectation](../../general/expected_value.md) to find the values in the grid.)

[zero-sum](../../general/zero-sum_game.md)

We call the options $a$, $b$, $c$, $x$ and $y$ strategies.

[Minmax decision](../../general/minmax_decision.md)

[Optimum play exists for 2-player zero-sum games with perfect information](../../general/optimum_play_exists_for_2-player_zero-sum_games_with_perfect_information.md)
...
[Mixed strategy](../../general/mixed_strategy.md)
...
In the [Prisoner's dilemma](../../general/prisoner's_dilemma.md) the dominating strategy is for them both to testify. Even if any player could change their decision after finding out the outcome they wouldn't want to.

[Nash equilibrium](../../general/nash_equilibrium.md)
...
This could be used to get [Nash equilibrium](../../general/nash_equilibrium.md).

[Elimination and Nash Equilibrium](../../general/elimination_and_nash_equilibrium.md)

Though this doesn't guarantee finding it. Though there is always a [Nash equilibrium](../../general/nash_equilibrium.md) in finite games.

[Existance of Nash equilibrium](../../general/existance_of_nash_equilibrium.md)
...
If the number of rounds are known beforehand, then consider the last round. As there are no games afterwards - there is no incentive to not behave rationally, so we will use the [Nash equilibrium](../../general/nash_equilibrium.md) strategy.

Though given this last game is already decided we might as well consider playing one less game. Though this inductively gives that each player will play a [Nash equilibrium](../../general/nash_equilibrium.md) strategy at each step of the game.
...
>[!Note]
>[Discounted rewards](../../general/discounted_rewards.md) is perfectly built for this. Suppose $G$ is the random variable for the value of playing this game, let $g_i$ be the reward for the $i'th$ round then
...
[Tit for Tat](../../general/tit_for_tat.md)

>[!example] [Prisoner's dilemma](../../general/prisoner's_dilemma.md)
...
Here no matter what $\gamma$ we have that testify is a [Nash equilibrium](../../general/nash_equilibrium.md). Though we have additional equilibrium depending on $\gamma$
...
[Minmax profile](../../general/minmax_profile.md)

[Battle of the sexes](../../general/battle_of_the_sexes.md)

To calculate the [Minmax profile](../../general/minmax_profile.md) of the [Battle of the sexes](../../general/battle_of_the_sexes.md) we have to think about how one player could punish the other player the most. (This game is simpler as it is symmetric.) Using a [Pure strategy](../../general/pure_strategy.md) if $B$ wanted to punish $A$ they would go for $y$ and so the best $A$ could do would be 1 point.
...
This gives the following [Minmax profile](../../general/minmax_profile.md).
> $$
 \begin{array}{c|c}
 \ \mbox{A} & \mbox{B} \\ \hline
 \frac{2}{3} & \frac{2}{3} \\
...
In the [Battle of the sexes](../../general/battle_of_the_sexes.md) example we have plotted the security region below.
...
[Grim trigger strategy](../../general/grim_trigger_strategy.md)

[Folk Theorem](../../general/folk_theorem.md)

If we follow through with the threat in the [Grim trigger strategy](../../general/grim_trigger_strategy.md) this might actually be sub-optimal play for us - and we might not want to do it. So this threat is some what implausible.

[Subgame perfect](../../general/subgame_perfect.md)
...
The strategy is in [Nash equilibrium](../../general/nash_equilibrium.md) with itself, as both players start out cooperating and will continue forever. However it is also [Subgame perfect](../../general/subgame_perfect.md) and therefore a plausible threat. To see this lets start a game in every previous state. We will represents turns as $(*,*)$ where the first position is what player $A$ did and the second is what player $B$ did. There are two options are moves cooperate $C$ and defect $D$.
> $$
 \begin{array}{c|cc}
 \ \mbox{Last} & 1 & 2 \\ \hline
 (C,C) & (C,C) & (C,C) \\
...
We see from this no matter what happened last round the [Pavlov strategy](../../general/pavlov_strategy.md) self rights itself into the cooperate cooperate state.
...
[Stochastic games](../../general/stochastic_games.md)
...
[Minimax-Q](../../general/minimax-q.md)
...
- $Q_i$ has a multiple solution for each $i$ (as there is no unique [Nash equilibrium](../../general/nash_equilibrium.md)),
...
- We can use repeated [Stochastic games](../../general/stochastic_games.md),
