...
name: Week 5 - Computational Learning Theory
tags:
  - OMSCS
type: lecture
week: 5
...
[Computational learning theory](../../general/computational_learning_theory.md) is fundamentally about 3 things:
...
3. Complexity of the hypothesis class.
	1. How complex is the representation - [modelling paradigm](../../general/modelling_paradigm.md)
4. Accuracy to which the target concept is approximated.
	1. Within some $\epsilon$.
5. Manner in which the examples are presented.
	1. Batch or online.
6. Manner in which the [training data](../../general/training_data.md) is collected.
...
[Run time complexity](../../general/run_time_complexity.md)

However there are some domain specific definitions as well.

[Sample complexity](../../general/sample_complexity.md)

Touching upon the example above.

[Mistake bound](../../general/mistake_bound.md)

Lets fix terminology for the next section. Suppose we are in the [modelling framework](../../general/modelling_framework.md) with some training data $T$.
- Given some [hypothesis space](../../general/modelling_paradigm.md) $H$ the "true hypothesis" is $c \in H$ (i.e. for all $(a,b) \in T$ $c(a) = b$).
- The current candidate hypothesis will be $h \in H$.
- A [consistent learner](../../general/consistent_learner.md) $h$ is one where $h(a) = c(a)$ for all $(a,b) \in T$.
- The [version space](../../general/version_space.md) VS(T) = \{$h \ \vert$  $h$ is a consistent learner with respect to $T$\}.

## Pac learning

There are two concepts of error when evaluating a hypothesis.

[Training error](../../general/training_error.md)

[True error](../../general/true_error.md)

Using this we can define what we call "probably approximately correct" (PAC) learner.

[PAC learner](../../general/probably_approximately_correct_learnable_(pac).md)

Though showing something is PAC-learner is hard.

>[!example] $k$-bit example
>Suppose our underlying set $X = \{0,1\}^k$ and $H = C = \{h_i : h_i(x) = x_i\}$. Is there an algorithm that is $\epsilon$, $\delta$ [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md)?

## $\epsilon$-exhausted version space

[epsilon-exhausted version space](../../general/epsilon-exhausted_version_space.md)

>[!example] Xor example
>Suppose $A = \{0,1\}^2$ and our real function is [xor](../../general/exclusive_or.md) $\oplus$ on two variables $x_1$ and $x_2$. Let $H = \{x_1, \overline{x_1}, x_2, \overline{x_2}, T, F, \land, \lor, \oplus, =\}$ and set probability distribution $\mathbb{D}(0,0) = 0.1$, $\mathbb{D}(0,1) = 0.5$, $\mathbb{D}(1,0) = 0.4$, and $\mathbb{D}(1,1) = 0$.
>Then with $T = \{((0,0), 0), ((1,0), 1)\}$ we have our [version space](../../general/version_space.md) to be $VS_H(T) = \{x_1, \lor, \oplus\} \subset H$.
>This [version space](../../general/version_space.md) is $0.5$-exhausted as $x_1$ doesn't correctly map $(0,1)$ and $\lor$ doesn't correctly map $(1,1)$. However as $\mathbb{D}(0,1) = 0.5$ and $\mathbb{D}(1,1) = 0$ a hypothesis in the [version space](../../general/version_space.md) has error at worst $0.5$.

This is useful concept for finding the number of samples we need to take to us to give us a [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md).

[Haussler Theorem](../../general/haussler_theorem.md)

Note in the definition of [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md) $\mathbb{P}[Error_{\mathbb{D}}(h) > \epsilon]$ is exactly the quantity we need to bound with $\delta$ (as we need the probability of finding a good learner to be $1 - \delta$). Therefore if we have the ability to draw [i.i.d.](../../general/independent_identically_distributed_samples.md) examples we need $m$ to obey
$$
...
$$
(Note this is polynomial in $\frac{1}{\epsilon}$, $\vert H \vert$ and $\frac{1}{\delta}$ as required by a [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md).) So given that good learner does exist in our [hypothesis space](../../general/modelling_paradigm.md) we have use drawing [i.i.d.](../../general/independent_identically_distributed_samples.md) representatives to give us a [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md) by checking if the hypothesis are consistent with the observations.

>[!example] 10-bit example
>Suppose our underlying set $X = \{0,1\}^{10}$ and $H = C = \{h_i : h_i(x) = x_i\}$. How many [i.i.d.](../../general/independent_identically_distributed_samples.md) samples do we need to draw to find a $\epsilon = 0.1$, $\delta = 0.2$ with $\mathbb{D}$ being uniform [PAC learner](../../general/probably_approximately_correct_learnable_(pac).md)?
...
A weakness of [Haussler Theorem](../../general/haussler_theorem.md) is that it doesn't handle infinite dimensional hypothesis spaces.
