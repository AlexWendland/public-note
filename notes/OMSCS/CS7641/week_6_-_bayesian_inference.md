...
name: Week 6 - Bayesian Inference
tags:
  - OMSCS
type: lecture
week: 6
...
>Note if $A$ and $B$ are [independent event](../../general/independent_events.md) then
...
[Bayesian network](../../general/bayesian_network.md)

$\mathbb{P}[X_1 = a_1, X_2 = a_2, .... X_n = a_n] = \prod_{i=1}^n \mathbb{P}[X_i = a_i \vert X_u = a_u \forall (u,v) \in E]$


>[!example] Independent variables
>Let $A$ and $B$ be [independent event](../../general/independent_events.md). Then $G = (\{A, B\}, \emptyset )$ forms a [Bayesian network](../../general/bayesian_network.md) as
>$$\mathbb{P}[A,B] = \mathbb{P}[A]\mathbb{P}[B].$$
>Note that it also forms a [Bayesian network](../../general/bayesian_network.md) with $G = (\{A, B\}, \{(A,B)\} )$ or $G = (\{A, B\}, \{(B,A)\} )$ but it would not be minimal.

>[!example] Conditionally independent
>Suppose we have [random variables](../../general/random_variable.md) $A$, $B$, and $C$ where $C$ is [conditionally independent](conditional_independence.md) of $A$ given $B$. Then $G = (\{A, B, C\}, \{(A,B), (B,C)\})$ forms as [Bayesian network](../../general/bayesian_network.md).

The property of being [conditionally independent](conditional_independence.md) is deducible from the [Bayesian network](../../general/bayesian_network.md) $(G, X)$ by using the [chain rule](../../general/chain_rule_(probability).md).

[chain rule](../../general/chain_rule_(probability).md)

Lets [topological order](../../general/topological_sorting_(dag).md) $V$ so $V = \{1, 2, \ldots, n\}$ where $(i,j) \in E$ we have $i < j$. Then apply the [chain rule](../../general/chain_rule_(probability).md) to
$$
...
This is in fact a defining property of [Bayesian network](../../general/bayesian_network.md).

[Local Markov property](../../general/local_markov_property.md)
...
[Marginalisation (probability)](../../general/marginalisation_(probability).md)

[chain rule](../../general/chain_rule_(probability).md)

[Statement](../../general/bayes_rule.md#statement)
...
>This can be represented as a [Bayesian network](../../general/bayesian_network.md) with the following graph $(\{P, D1, D2\}, \{(P, D1), (D1, D2), (P, D2)\})$. Note this means $D1$ depends on $P$ and $D2$ depends on $P$ and $D1$.
>What is $\mathbb{P}[D2 =$ blue $\vert D1 =$ green$]$?

We can write out the [conditional probability](../../general/conditional_probability.md) tables when they rely on all their dependents quite easily. First note $\mathbb{P}[P = 1] = \mathbb{P}[P = 2] = 0.5$. For the first joint distribution, we have what is below.
...
These tables perfectly set us up to calculate an exact state using the [chain rule](../../general/chain_rule_(probability).md). So we are going to try and rewrite this probability in terms of exact states first. So lets use the definition of [conditional probability](../../general/conditional_probability.md)
$$\mathbb{P}[D2 = blue \vert D1 = green] = \frac{\mathbb{P}[D2 = blue, D1 = green]}{\mathbb{P}[D1 = green]}$$
Next lets use [marginalisation](../../general/marginalisation_(probability).md) to add in the first variable.
...
Lastly lets use the [chain rule](../../general/chain_rule_(probability).md) to turn them into the probabilities we have in the tables above. For example
...
To turn this into something we can use from above we apply [Bayes rule](../../general/bayes_rule.md) to this
$$\mathbb{P}[S \vert V, \lnot P, \lnot U] = \frac{\mathbb{P}[V, \lnot P, \lnot U \vert S] \mathbb{P}[S]}{\mathbb{P}[V, \lnot P, \lnot U]}.$$
(Shortening the words to their first letters.) Next we apply the [chain rule](../../general/chain_rule_(probability).md) but knowing that $V$ $P$ and $U$ are [conditionally independent](conditional_independence.md).
$$
\begin{align*}
\mathbb{P}[S \vert V, \lnot P, \lnot U] & = \frac{\mathbb{P}[V, \lnot P, \lnot U \vert S] \mathbb{P}[S]}{\mathbb{P}[V, \lnot P, \lnot U]}\\
...
[Naive Bayes classifier](../../general/naive_bayes_classifier.md)

This is cool as,
- Normally [Inference](../../general/inference.md) is expensive to do, however in this case it makes it really cheap.
- The model has few parameters so has low [spacial complexity](../../general/spacial_complexity.md).
- If you have sufficient data you can make very good estimates on these probabilities.
- This connects [Inference](../../general/inference.md) and [classification problems](../../general/classification_problems.md).
