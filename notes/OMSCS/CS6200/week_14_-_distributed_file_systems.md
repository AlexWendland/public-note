...
name: Week 14 - Distributed file systems
tags:
  - OMSCS
type: lecture
week: 14
...
However with [DFS](../../general/depth-first_search_(dfs).md) we have much larger network latency than in [CPU](../../general/central_processing_unit_(cpu).md) so the question about when this happens is important:
...
- To support the above [Distributed file system (DFS)](../../general/distributed_file_system_(dfs).md) can introduce flush/sync operations.
...
This application has changed over the years, NFSv3 is stateless whilst NFSv4 is stateful. The change made [NFS](../../general/network_file_system_(nfs).md) to support caching and locking.
