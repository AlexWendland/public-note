...
- Manage what should be in [physical memory](../../general/physical_memory.md).
- Validate memory access of processes.

When we speak about memory there are two main abstractions [OS](../../general/operating_system_(os).md) can use for memory:
- [Paging system](../../general/paging_system.md): This uses [memory pages](../../general/memory_page.md) which fixes the size of the smallest unit that can be moved between memory locations. This is the currently the most popular way to manage memory.
- [Memory segmentation](../../general/memory_segmentation.md): This uses [memory segments](../../general/memory_segment.md) which have variable sizes.

To support these memory mapping there are 3 bits of hardware that take a [virtual address](../../general/virtual_memory.md) to a place on [RAM](../../general/random_access_memory_(ram).md).
- [Memory Management Unit (MMU)](../../general/memory_management_unit_(mmu).md): A component on the [CPU](../../general/central_processing_unit_(cpu).md) that performs the mapping from [virtual memory](../../general/virtual_memory.md) to [physical memory](../../general/physical_memory.md). This is also responsible for raising faults for illegal access or accesses that require different permissions.
...
Note here in [paging](../../general/paging_system.md) that the physical address consists of a [virtual page number](../../general/virtual_page_number_(vpn).md) with an offset.

>[!warning] Offset index
>The minimum addressable unit of memory is a [byte](../../general/byte.md) not a [bit](../../general/bit.md). So the offset within a [page](../../general/memory_page.md) is given in [bytes](../../general/byte.md).

[Physical memory](../../general/physical_memory.md) is only allocated to [virtual memory](../../general/virtual_memory.md) when it is first referenced. This is identified by the operating system when a [physical memories](../../general/physical_memory.md) location is not in a [page table](../../general/page_table.md). When this happens the [operating system](../../general/operating_system_(os).md) takes control and either allocates the memory or swaps something out of [RAM](../../general/random_access_memory_(ram).md) to provide space for the new memory. This is called 'allocation on first touch'. When a [page](../../general/memory_page.md) is removed from [RAM](../../general/random_access_memory_(ram).md) this is called 'reclaimed'.

[Page table entry](../../general/page_table_entry.md)
...
- The size of the [pages](../../general/memory_page.md). I.e. What size chunks are you cutting your [RAM](../../general/random_access_memory_(ram).md) into.

The register size is important as it limits the size of the [virtual address](../../general/virtual_memory.md). The size of the page is important as it determines how large the offset needs to be.
...
>As we have a 32-bit architecture the [virtual addresses](../../general/virtual_memory.md) have size 32 [bits](../../general/bit.md).
>As the page size is 4kB = $2^2 * 2^{10} = 2^{12}$B we will need 12 bits for the offset.
>Therefore we are left with 20 bits of the [virtual address](../../general/virtual_memory.md) for the [Virtual page number (VPN)](../../general/virtual_page_number_(vpn).md). This means there are $2^{20}$ [pages](../../general/memory_page.md).
>With this we can now work out the size of the page table. For this architecture, 32-[bit](../../general/bit.md) addresses are 4 [bytes](../../general/byte.md) large which is the size of the [page table entry](../../general/page_table_entry.md). We have $2^{20}$ page entries so we get $4 * 2^{20}$B = 4 MB of size.
...
>Suppose we have the same processes running only a 12-[bit](../../general/bit.md) architecture. Though we run it on two different machines where:
>- the first uses flat page tables, with [virtual addresses](../../general/virtual_memory.md) having a 6 bit [VPN](../../general/virtual_page_number_(vpn).md) and a 6-bit offset, and
>- the second uses 2-level page tables, with the [virtual addresses](../../general/virtual_memory.md) having a 2 bit first index and 4 bit second index with a 6-bit offset.
>
...
[descriptor table](../../general/descriptor_table.md)
...
The main challenge [memory allocators](../../general/memory_allocator.md) suffer from is [external fragmentation](../../general/external_fragmentation.md).

[External fragmentation](../../general/external_fragmentation.md)

The linux kernel has two types of allocators.

[Buddy Allocator](../../general/buddy_allocator.md)

However, the objects the [linux](../../general/linux.md) kernel normally stores are not powers of 2. Causing a lot of [internal fragmentation](../../general/internal_fragmentation.md) so another [memory allocator](../../general/memory_allocator.md) is also used.
...
- History-based predictions such as [LRU](least-recently_used_(lru).md)
	- Access bit in the [page table entry](../../general/page_table_entry.md) can be used for this.
