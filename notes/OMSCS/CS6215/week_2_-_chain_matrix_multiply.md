...
The [recursion](../../general/recursion.md) step will be
$$ C(i,j) = \min\{C(i,k) + C(k,j) + m_im_km_j \vert i < k < j\}.$$

To calculate $C(i,j)$ we need to [iteratively](../../general/iterative_algorithms.md) increase the difference between $j - i$, the base case is $0$ the last case will be $n-1$.
...
>Given a sequence $a_1, a_2, \ldots, a_n$ devise an algorithm that returns the length of the longest [palindromic](../../general/palindrome.md) [subsequence](../../general/subsequence.md). Its running time should be $O(n^2)$.
