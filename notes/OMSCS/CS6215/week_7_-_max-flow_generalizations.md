...
name: Week 7 - Max-flow Generalizations
tags:
  - OMSCS
type: lecture
week: 7
...
We would like to turn the [flow network](../../general/flow_network.md) with demands into a problem of just the [max flow](../../general/max_flow_problem.md).
...
We will define a new [flow network](../../general/flow_network.md) $(G' = (V',E'), c', s', t')$ to do this. First lets define the graph $G'$ with $V' = V \cup \{s', t'\}$ and
...
Then a [flow](../../general/flow.md) $f'$ on $(G', c', s', t')$ will be converted into a flow on $(G,c,s,t)$ by defining $f(e) = f'(e) + d(e)$ for $e \in E$.

However, this will only be a valid [flow](../../general/flow.md) on $(G,c,s,t)$ if the value of $capacity(f') = \sum_{e \in E} d(e)$ (the maximum capacity on $(G', c', s', t')$ ). Such a [flow](../../general/flow.md) $f'$ will be called a saturating [flow](../../general/flow.md).
...
We run [Ford-Fulkerson Algorithm](../../general/ford-fulkerson_algorithm.md) or [Edmonds-Karp algorithm](../../general/edmonds-karp_algorithm.md) but instead of starting from a zero flow we instead start from $f$.

Also instead of defining the usual [residual network](../../general/residual_network_(flow).md) instead we define a limited version
