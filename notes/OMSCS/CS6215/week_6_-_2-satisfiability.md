...
[Statement](../../general/satisfiability_problem_(sat_problem).md#statement)
...
[Statement](../../general/k-satisfiability_problem_(k-sat_problem).md#statement)
...
Suppose the [path](../../general/path_(graph).md) is of length 1, i.e. $(a, b) \in E$. Therefore there is a clause $\overline{a} \lor b$ or $b \lor \overline{a}$, so if $a$ is true then $b$ needs to be true.

Suppose it is true for a path of length less than $k$.

Suppose we have a [path](../../general/path_(graph).md) of length $k>1$ $p (c,b)$ where $p$ is a [path](../../general/path_(graph).md) from $a$ to $c$ of length $k-1$.
...
This means in $G$ it is meaningful to talk about complement paths. i.e. if $p_{a,b}$ is a [path](../../general/path_(graph).md) in $G$ from $a$ to $b$ then $\overline{p_{a,b}}$ is the complement path from $\overline{b}$ to $\overline{a}$ that we will refer to as $p_{\overline{b}, \overline{a}} := \overline{p_{a,b}}$.
...
By the Lemma above $\overline{p_{1,2}}$ and $\overline{p_{2,1}}$ connects $\overline{s_2}$ to $\overline{s_1}$ and $\overline{s_1}$ to $\overline{s_2}$. Showing that $\overline{s_1}$ and $\overline{s_2}$ are [strongly connected](../../general/strongly_connected_(directed_graphs).md) to one another.

Equally if $\overline{t} \in V$ is [strongly connected](../../general/strongly_connected_(directed_graphs).md) to $\overline{s} \in \overline{S}$ via paths $\overline{p_{t,s}}$ and $\overline{p_{s,t}}$ then $p_{t,s}$ and $p_{s,t}$ strongly connect $t$ to $s$ giving $t \in S$.

Therefore $\overline{S}$ is a [strongly connected component](../../general/strongly_connected_components_(directed_graphs).md) of $G$.
...
From the Corollary above we know $\overline{S}$ is a [strongly connected component](../../general/strongly_connected_components_(directed_graphs).md).
...
We can pair the [strongly connected components](../../general/strongly_connected_components_(directed_graphs).md) $S$ with $\overline{S}$. From the assumption of the lemma $S \not = \overline{S}$ for any [strongly connected component](../../general/strongly_connected_components_(directed_graphs).md).

To satisfy $f$ we need to find a True/False allocation to the [strongly connected components](../../general/strongly_connected_components_(directed_graphs).md) such that:
...
Constructing the graph takes $O(m)$ steps. Then running the [SCC algorithm](../../general/2-sat_algorithm_using_scc.md) takes $O(n + m)$. Lastly filtering the [strongly connected components](../../general/strongly_connected_components_(directed_graphs).md) takes at most $O(n)$ steps as we have to run through the list of variables and their complements. (Note the ordering [SCC algorithm](../../general/2-sat_algorithm_using_scc.md) algorithm returns in is a [topologically sort](../../general/topological_sorting_(dag).md) so it isn't hard to identify sink components.)
...
[2-SAT algorithm using SCC](../../general/2-sat_algorithm_using_scc.md#2-sat-algorithm-using-scc)
...
[Run time](../../general/2-sat_algorithm_using_scc.md#run-time)
