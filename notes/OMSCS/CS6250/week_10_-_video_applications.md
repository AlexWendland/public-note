...
name: Week 10 - Video applications
tags:
  - OMSCS
type: lecture
week: 10
...
Whilst video does have a much larger bit-rate you may be ok with skipping [bits](../../general/bit.md) or there being a slight delay - in contrast with voice where skipped  [bits](../../general/bit.md) or delayed audio may make the stream unusable. Therefore they have different dynamics at play.
...
[VoIP](../../general/voice_over_ip_(voip).md) faces unique challenges as the [Internet](../../general/internet.md) is best effort delivery and does not come with built in quality of service gaurentees. Moveover, audio is an analogue form so any [VoIP](../../general/voice_over_ip_(voip).md) will need to send a digital approximation.

- **Encoding**: We normally encode sound by making lots of descrete apprximations of the waves current position this is call **[quantization](../../general/quantization.md)**. There are different categories of encoding schemes, narrowband, broadband, and multimode these each have different trade offs. The encoding hopes to make speech intelligible but also use as little bandwidth as possible.
- **Signalling**: Traditionally this takes care of how calls are set up and torn down. [Protocols](../../general/protocol_(networks).md) such as [Session Initiation Protocol (SIP)](../../general/session_initiation_protocol_(sip).md) had handle this in [VoIP](../../general/voice_over_ip_(voip).md) applications. This has four major responsibilities.
...
To avoid packet loss we might want to use [TCP](../../general/transmission_control_protocol_(tcp).md) however due to the tougher definition the transmission of packets might arrive after the jitter window so be considered lost anyway. Therefore most [VoIP](../../general/voice_over_ip_(voip).md) applications use [UDP](../../general/user_datagram_protocol_(udp).md) and just accept some packet loss - this lowers bandwidth usage as well.
...
Instead the [HTTP](../../general/hyper_text_transfer_protocol_(http).md) protocol was already well used and understood by network participants. Here the server can be stateless and just provide the content the client requests. This had the following advantages:
- Servers can be stateless.
- Providers could use already established [CDNs](../../general/content_delivery_network_(cdn).md)
...
[Bitrate](../../general/bitrate.md)

Therefore a single [bitrate](../../general/bitrate.md) would not handle this . Therefore normally producers make encodings for different required [bitrates](../../general/bitrate.md)for streaming, such as 250 kbps, 500 kbps, 1.5 Mbps, 3 Mbps. Then we us [bitrate adaption](../../general/bitrate_adaption.md) to adjust the provided quality to the user depending on network and device requirements.
...
3. Low video quality variations: i.e. a change in the video quality caused by [bitrate adaption](../../general/bitrate_adaption.md).
...
- Different encoding that may require higher [bitrate](../../general/bitrate.md) for shorter periods.
- Application and [Transport layer](../../general/layer_4_transport.md) overhead.

When forecasting $C(t)$ we normally use a weighted average of the past [bitrate](../../general/bitrate.md). This can be sluggish to adapting to a fall in [bitrate](../../general/bitrate.md) causing rebuffering. Though the bigger issue is with how this interacts with [TCP](../../general/transmission_control_protocol_(tcp).md).
