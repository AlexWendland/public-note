...
[Internet Service Provider (ISP)](../../general/internet_service_provider_(isp).md)

[Content delivery network (CDN)](../../general/content_delivery_network_(cdn).md)

[Internet Exchange Points (IXPs)](../../general/internet_exchange_points_(ixps).md)
...
The cost to [ISPs](../../general/internet_service_provider_(isp).md) is maintaining a network with sufficient capacity to handle all that traffic. Therefore unless traffic is going to a [host](../../general/host_(networks).md) paying them they will need a commercial incentive to carry it.

Therefore two types of relationships form between [ISPs](../../general/internet_service_provider_(isp).md)
- **Peering relationships**: If two providers see [hosts](../../general/host_(networks).md) exchange traffic between there networks in roughly equal quantities they may agree to share traffic directly at no cost between the [ISPs](../../general/internet_service_provider_(isp).md).
- **Customer-Provider relationships**: If one [ISP](../../general/internet_service_provider_(isp).md) has considerably more [hosts](../../general/host_(networks).md) receiving traffic then they can sell access to these hosts to other [ISPs](../../general/internet_service_provider_(isp).md). Here the larger [ISP](../../general/internet_service_provider_(isp).md) becomes the provider to the smaller customer [ISP](../../general/internet_service_provider_(isp).md) and picks up a fee.
...
- **Routes form customers**: These are profitable for an [ISP](../../general/internet_service_provider_(isp).md) to share as they generate revenue from more traffic going to these. These are shared willingly to all.
...
[Boarder gateway protocol (BGP)](../../general/boarder_gateway_protocol_(bgp).md)

The goals of [BGP](../../general/boarder_gateway_protocol_(bgp).md) are:
- **Scalability**: How do you maintain a routing table when the size of the internet is expanding.
- **Expressing route policies**: How do we allow for [AS](../../general/autonomous_system_(as).md) to control which routes to use and to broadcast.
...
- eBGP: For communicating with other [AS](../../general/autonomous_system_(as).md).

There are three important messages [BGP](../../general/boarder_gateway_protocol_(bgp).md) has.
...
In the [BGP](../../general/boarder_gateway_protocol_(bgp).md) the [routers](../../general/router.md) exchange [IP prefixes](../../general/subnets.md) that represent [subnets](../../general/subnets.md) or collections of [subnets](../../general/subnets.md) if the [router](../../general/router.md) is using [route summarization](../../general/route_summarization.md). For the routes agreed by the [AS](../../general/autonomous_system_(as).md) the [router](../../general/router.md) offers theses over [eBGP](../../general/boarder_gateway_protocol_(bgp).md) and then shares the routes it has been offered over [iBGP](../../general/boarder_gateway_protocol_(bgp).md).

Messages passed between [AS](../../general/autonomous_system_(as).md) have some special properties, two of which are:
...
To limit the size of the routing table it can filter out routes that are too specific. This encourages [route summarization](../../general/route_summarization.md). The act of [route summarization](../../general/route_summarization.md) protects the whole network from getting overloaded and help with scalability. Small [AS](../../general/autonomous_system_(as).md) sometimes just use a default gateway where they redirect all traffic without further knowledge.
...
- A monthly fee for renting a port - this cost normally scales based on the speed/capacity of that [port](../../general/port.md).
- Sometimes there is a yearly subscription fee.

Once connected to the [IXP](../../general/internet_exchange_points_(ixps).md) there is normally no cost to publicly peer there. That means getting access to all the other networks also publicly peering there.
...
3. **Route servers and Service level agreements**: Normally the [IXP](../../general/internet_exchange_points_(ixps).md) will offer free access to a route server which is a giant public route table. The [IXP](../../general/internet_exchange_points_(ixps).md) will also offer [SLAs](../../general/service_level_agreements_(slas).md) with the services they offer.
...
- It execute [BGP](../../general/boarder_gateway_protocol_(bgp).md) decisions and re-advertises the resulting information.

The collection of addresses is called a Routing Information Base (RIB) which contains all [BGP](../../general/boarder_gateway_protocol_(bgp).md) paths. There is a master RIB with all the information and an [AS](../../general/autonomous_system_(as).md) specific RIB for each participant.

Route servers maintain two types of route filters. Import route filters that allow [ASs](../../general/autonomous_system_(as).md) to only advertise routes they should advertise. Export route filters which are triggered by member [ASs](../../general/autonomous_system_(as).md) to restrict which other [IXP](../../general/internet_exchange_points_(ixps).md) members can receive their routes.
...
1. First AS X advertises a prefix p1 to the route server, which is added to the route server's RIB for that [AS](../../general/autonomous_system_(as).md).
