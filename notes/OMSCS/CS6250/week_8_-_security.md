...
[Round robin DNS (RRDNS)](../../general/round_robin_dns_(rrdns).md)

[DSN-based content delivery](../../general/dsn-based_content_delivery.md)
...
[Finding rouge networks (FIRE)](../../general/finding_rouge_networks_(fire).md)
...
This system predicts the likelihood of a security breach within an organisation using externally observable features. It uses these features to train a [Random forest](../../general/random_forest.md) model, making it callable to all organizations. The features fall into three main categories:

1. **Mismanagement Symptoms**:
    - **Open Recursive Resolvers**: Misconfigured [DNS](../../general/domain_name_system_(dns).md) resolvers that can be exploited.
...
- **Human error**: Accidental misconfiguration of routers can lead to this type of attack. E.g. China Telecom accidentally leaked a full [BGP](../../general/boarder_gateway_protocol_(bgp).md) table that led to large-scale [Type-0 hijacking](../../general/type-0_hijacking.md)
...
[Spoofing](../../general/spoofing.md)

In the context of [DDoS](../../general/distributed_denial-of-service_(ddos).md) attacks this can be done in two ways:
...
[Access control list (ACL) filters](../../general/access_control_list_(acl)_filters.md)

[BGP Flowspec](../../general/bgp_flowspec.md)
...
By understanding and leveraging [BGP Flowspec](../../general/bgp_flowspec.md), network administrators can better protect their infrastructures from [DDoS](../../general/distributed_denial-of-service_(ddos).md) attacks, ensuring smoother and more secure network operations.

Lastly we consider [Blackholing (BH)](../../general/blackholing_(bh).md).

[blackholing](../../general/blackholing_(bh).md)

[BGP Blackholing](../../general/bgp_blackholing.md)

When implemented in an [IXP](../../general/internet_exchange_points_(ixps).md) if one of the participants does not accept the [BH](../../general/blackholing_(bh).md) message then this can render the who excise worthless.
