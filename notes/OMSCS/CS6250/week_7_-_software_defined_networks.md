...
- Proprietary Technologies: A lot of the software ran on [routers](../../general/router.md), [switches](../../general/switch.md), and [middleboxes](../../general/middleboxes.md) was closed and configuration interfaces varied between vendors.
...
- Proposing unified control over [middleboxes](../../general/middleboxes.md).
...
The main role of the data plane is the forwarding of packets based on some [FIB](../../general/forwarding_information_base_(fib).md), whereas the main role of the control plane is [routing](../../general/routing.md) which mainly involves updating the [FIB](../../general/forwarding_information_base_(fib).md) once it has computed the route a particular packet should take.
...
In [OpenFlow](../../general/openflow.md) how they handle packets is based on a flow table which has 3 entries
- A matching rule
...
[OpenFlow](../../general/openflow.md) allows for 3 information sources:
...
These could be addressed using [SDN](../../general/software_defined_networks_(sdn).md).

This has been implemented in [IXPs](../../general/internet_exchange_points_(ixps).md) using SDX. This allows for:
...
- Traffic redirection through [middleboxes](../../general/middleboxes.md) - Targeted subsets of traffic can be redirected to [middleboxes](../../general/middleboxes.md).

At an [IXP](../../general/internet_exchange_points_(ixps).md) each participant connects to the route server - there they get a virtual sdn switch where they decide the in/outbound rules for traffic leaving and entering their network. These do not effect other participants switches but does effect how traffic is routed through the [IXP](../../general/internet_exchange_points_(ixps).md). This differs from a standard route server which uses [BGP](../../general/boarder_gateway_protocol_(bgp).md) rules to control traffic form/to participant [AS](../../general/autonomous_system_(as).md).
