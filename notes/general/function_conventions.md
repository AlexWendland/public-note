...
[Switch statements](switch_statements.md) are one thing and should appear only once. Normally these switch statements will be used to create a [polymorphic](polymorphism.md) object and are hidden behind an [inheritance](inheritance.md) relationship so the rest of the system can't see it. Look at the [constructor pattern](constructor_pattern.md) to see example.
...
A [side effect](side_effect.md) is a secondary effect not made clear by the functions name. For example checking if the function checked the state is in some position and then changed the state some how. This is bad practice as it makes it less clear to the user what they are doing and may have unexpected consequences.
...
The best functions are ones with no arguments (niladic). This makes it easier to read as regularly arguments are at a different level of abstraction to the function. Also easier for [testing](testing_conventions.md) as you don't have to try the function with multiple different arguments.
...
Having to handle null input is ugly and unreadable and it is an indicator that your function is most probably having to do too much. Why do you need the null argument? If you have some conditional logic should the be split into separate functions and chosen via a [Factory Pattern](factory_pattern.md).

### Make function variables instance variables

The arguments you might want to pass to the function could be instance variables. (This is a very [object oriented](object_oriented_programming_(oop).md) approach.)
...
Most programmers won't expect the output to be given by an input variable such as using a [mutable](mutability.md) variable. Provide the output as a new variable.
...
Throw exceptions, don't pass error codes from your function. It means when you get a return from a function you have to check it for a error type. As well as it makes typing harder to do. This also subtly breaks [Command Query separation](function_conventions.md#command-query-separation) separation.

## Function organisation

### Use descriptive names

The name of the function should tell you exactly what it does. Look at [naming conventions](naming_conventions.md) to see how to do it.
