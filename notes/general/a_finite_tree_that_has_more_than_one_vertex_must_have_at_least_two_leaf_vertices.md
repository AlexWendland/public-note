...
>A finite [graph](graph.md) who has more than one vertex that is a [tree](tree_(graph).md) must have at least two leaf vertex.

## Proof

Proof by contradiction.

Suppose this is not the case, then if it has a single vertex of degree one start a path at this vertex. Otherwise pick a random vertex to start the path at. As the tree is connected and has more than one vertex the [degree](degree_(graph).md) of the chosen vertex is at least 1. It then has an edge to another vertex or itself.
...
The next vertex then must have another edge incident to it as it has [degree](degree_(graph).md) at least 2. This edge is either incident to a vertex we have already visited or lead to a vertex we have not visited.

If it is incident to a vertex we have already visited we have a [cycle](cycle_(graph).md) in our graph and this is not a tree.
