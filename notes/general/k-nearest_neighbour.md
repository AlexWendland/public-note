...
In the [modelling framework](modelling_framework.md), suppose we are given:
- [training data](training_data.md) $T \subset (A,B)$,
- a [metric](metric.md) on $A$ called $d: A \times A \rightarrow \mathbb{R}$,
- $k \in \mathbb{Z}$ to be the number of points to consider, and
- an averaging mechanism $V: \mathcal{P}(B \times \mathbb{R}) \rightarrow B$ that takes a weighted set of values in $B$ and returned another value in $B$ - like the weighted [geometric mean](geometric_mean.md).
...
- Input points are represented by the [training data](training_data.md).
