...
name: Every min-cut is at full capacity in a max-flow
tags:
  - programming
type: lemma
---
...
>[!important] Lemma
>Given a [Flow network](flow_network.md) $(G, c, s, t)$ with a [max flow](max_flow_problem.md) $f$. Then for every [min st-cut](min_st-cut_problem.md) ($S$, $T$) all edges in $cut(S,T)$ have maximum capacity in $f$.
...
Note this also gives [every min-cut has no flow going backwards along it in a max-flow](every_min-cut_has_no_flow_going_backwards_along_it_in_a_max-flow.md).
