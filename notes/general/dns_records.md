...
>2. **AAAA (Quad-A) Record**: Maps a domain name to an [IPv6](internet_protocol_(ipv6).md) address.
...
>10. **CAA (Certification Authority Authorization) Record**: Specifies which [certificate authorities](certificate_authorities_(ca).md) are allowed to issue certificates for the domain.
