...
name: Congestion control in TCP
tags:
  - networks
type: explainer
---
...
[TCP](transmission_control_protocol_(tcp).md) uses the end-to-end approach. It mainly uses packet loss to detect congestion as packet delay can have quite a bit of noise in the system. It uses major network delay to reset itself.

[TCP](transmission_control_protocol_(tcp).md) uses ACK messages as a probes and adopts a probe-and-adapt approach to adjusting the window size (number of unacknowledged packets) for congestion control.

[Additive increase Multiplicative Decrease (AIMD)](additive_increase_multiplicative_decrease_(aimd).md)

[TCP Reno](tcp_reno.md)

## Fairness

This achieves fairness through [AIMD](additive_increase_multiplicative_decrease_(aimd).md). As the punishment for dropping a packet is exponential vs an increase which is linear. If you are using more of the network your probability of dropping a packet is higher and equally that punishment is larger. Whilst you are not dropping packets your increase is the same as every other network participant.
...
$$
>[!Note] Data Centres
>In Data Centres we see a lot of developments in [TCP](transmission_control_protocol_(tcp).md) implementations for congestion control. There are two main reasons for this:
>- Data centres make lots of delay sensitive short messages that normal [TCP](transmission_control_protocol_(tcp).md) wouldn't handle well.
