...
def update_last_edited(obsidian_file: models.ObsidianFile):
    ...
    if not last_edited:
        last_edited = datetime.date.fromtimestamp(Path(obsidian_file.file_path).stat().st_mtime)
        logger.info(
...
def get_last_edited(file_path, repo_path=constants.OBSIDIAN_DIR) -> datetime.date | None:
    repo = Repo(repo_path)
...
def update_tags(obsidian_file: models.ObsidianFile):
    old_tags_raw = obsidian_file.metadata.get("tags", None)
