...
ALIAS_LINKS : Dict[str, ObsidianLink] = dict()

FILE_ALIASES : Dict[str, Set[ObsidianLink]] = dict()
...
def _clean_string_of_aliases(text: str) -> Tuple[str, List[ObsidianLink]]:
    ...
    for match in matches:
        ...
        text = re.sub(fr'(?<!\!)\[\[{match}\]\]', link.clean_representation(), text)
        links.append(link)
    print("Post-clean string:", text)
    return text, links

def build_rehydration_map(alias_links = ALIAS_LINKS) -> dict:
    """
    Builds a map of all aliases to their link.
    """
    rehydration_dictionary = dict()
...
def _rehydrate_string(text: str, rehydration_dictionary: dict) -> str:
    ...
    while text:
        ...
        if current_alias:
            ...
            modified_text = (
                modified_text[:modified_index] + link.link() + modified_text[modified_index + len(current_alias):]
            )
            text = text[len(current_alias):]
            modified_index += len(link.link())
        else:
            text = text[1:]
            modified_index += 1
